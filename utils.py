...
class Image_Helper:
    ...
    def display_coordinates(self,event, x, y, flags, params):
        if event == 2:
            if not isinstance(self.active_channel,int):
                ...
                print(f"Coordinates: {x, y}; Color is {Image_Helper.color_to_channel_map[self.active_channel]} : {self.img[y,x,self.active_channel]}")
    ...
    def show_multiple_images(images: list, captions: list = None, step_by_step: bool = False,
        ...
        if step_by_step:
            for i in range(len(images)):
                images[i].show(name=str(i) if not captions else captions[i],show_coordinates=show_coordinates,channel=channel)
        else:
            for i in range(len(images)):
                cv2.imshow(str(i) if not captions else captions[i], images[i].img[:,:,channel] if isinstance(channel,int) else images[i].img)
            cv2.waitKey(0)
            cv2.destroyAllWindows()
    ...
    def shift(self, vertical_shift=0, horizontal_shift=0):
        if vertical_shift >= 0 and horizontal_shift >= 0:
            return Image_Helper(img=np.pad(self.img, ((vertical_shift, 0), (horizontal_shift, 0), (0, 0)))[:self.shape[0], :self.shape[1], :])
        elif vertical_shift < 0 and horizontal_shift >= 0:
            return Image_Helper(img=np.pad(self.img, ((0, -vertical_shift), (horizontal_shift, 0), (0, 0)))[-vertical_shift:, :self.shape[1],
                   :])
        elif vertical_shift >= 0 and horizontal_shift < 0:
            return Image_Helper(img=np.pad(self.img, ((vertical_shift, 0), (0, -horizontal_shift), (0, 0)))[:self.shape[0], -horizontal_shift:,
                   :])
        else:
            return Image_Helper(img=np.pad(self.img, ((0, -vertical_shift), (0, -horizontal_shift), (0, 0)))[-vertical_shift:,
                   -horizontal_shift:, :])
